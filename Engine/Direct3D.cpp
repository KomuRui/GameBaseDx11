...
namespace Direct3D
{
	...
	//二つ目のカメラ
	XMFLOAT3 pos = XMFLOAT3(900, 500, 900);
	XMFLOAT3 tar;
	bool        cameraFlag = false;
	int status = 0;                         //今描画してるのが一つ目のカメラなのかどうか 0:主 1:サブ
	...
	void Direct3D::SetCameraPos(XMFLOAT3 a)
	{
		pos = a;
	}

	void Direct3D::SetCameraTar(XMFLOAT3 a)
	{
		tar = a;
	}

	XMFLOAT3 Direct3D::GetCameraTar()
	{
		return tar;
	}

	XMFLOAT3 Direct3D::GetCameraPos()
	{
		return pos;
	}

	void Direct3D::CameraSet(bool flag)
	{
		cameraFlag = flag;
	}

	bool Direct3D::GetCamera()
	{
		return cameraFlag;
	}
